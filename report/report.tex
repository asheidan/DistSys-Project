...
%-----------------------------------------------------------
\section{Realisation of the project plan}
...
Below our requirement specification from out project plan is included in appendix \vref{app:reqspec}. We have succeded with implementing all required specifications. 
...
\item If the sequencer crashes all groups using the sequencer must be restarted since the message cache is lost and messages will get new serial numers starting at 1. The sequencer is placed at the same host as the registry wich is assumed to be relativle stable for this to not be a big problem.
